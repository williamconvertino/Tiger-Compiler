...
struct
    ...
    fun dataflow2interference dataflowGraph =
        let val liveness = Liveness.dataflow2liveness dataflowGraph
            ...
            fun addEdges (graph, temp, liveTemps) = S.foldl (fn (liveTemp, graph) => Graph.doubleEdge(graph, liveTemp, temp)) graph liveTemps
            ...
            Flow.Graph.foldNodes 
                (fn (node, (graph, moves)) => (
                    let val {li, lo} = Liveness.LivenessMap.lookup(liveness, (Flow.Graph.getNodeID node))
                        ...
                        val (graph', moves', lo') = List.foldr procInstr (graph, moves, lo) instrs
                    in
                        (graph', moves')
                    end
                ))
    ...
    fun printGraph (graph, moves) = (
        print("INTERFERENCE\n");
        Graph.printGraph (fn (id) => (Int.toString id)) (fn (id, node) => (Int.toString id)) graph;
        print("MOVES\n");
        Graph.printGraph (fn (id) => (Int.toString id)) (fn (id, node) => (Int.toString id)) moves
    )
end
