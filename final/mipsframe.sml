...
sig 
  type access
  type register
  type frame = Temp.label * access list * int ref * Tree.exp list * int ref
...
structure MipsFrame : FRAME = struct 
    ...
    fun newFrame {name, formals} =
      ...
      in 
        (name, formals, ref 0, [], ref 0)
      end

    fun name (name, _, _, _, _) = name
    fun formals (_, formals, _, _, _) = formals
    
    fun maxArgs (_, _, _, _, maxArgs) = maxArgs
    ...
    fun allocLocal (_, _, locals, _, _) true = (locals := !locals + 1 ; InFrame (!locals * ~4))
    ...
    fun procEntryExit1(body, frame) = 
      let val (label, formals, numLocals, _, _) = frame
